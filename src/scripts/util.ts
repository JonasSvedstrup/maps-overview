...
export const formatMapName = (str: string) =>
  str.replace("-", " ").replace(/(^\w{1})|(\s+\w{1})/g, (letter) => letter.toUpperCase());
export const formatMapShort = (str: string) => str.replace(" ", "-").toLowerCase();
export const formatMapLink = (str: string) => `map?name=${str.toLowerCase().replace(" ", "-")}`;
export const formatNumer = (number: number) => (number ? number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : "");
export const linkField = (link: string, value: string) => `<a href="${link}">${value}</a>`;
...
export const mapViews: string[] = ["top", "tiles", "start", "resources", "height", "wind", "fish", "transport"];
