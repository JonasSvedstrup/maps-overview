...
import { Connections, formatMapName, formatNumer, getSelectedMap } from '@/scripts/util';
...
const mapData: MapsData = rawMapsData.filter(map => map.name == formatMapName(selectedMap))[0];
...
if (connectionsString.endsWith(', ')) {
  connectionsString = connectionsString.substring(0, connectionsString.length - 2)
}
